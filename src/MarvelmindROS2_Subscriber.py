...
class GPSNode(Node):
    def __init__(self):
        ...
        # Timer to periodically get data
    ...
    def timer_callback(self):
        try:
            ...
            if self.hedge.positionUpdated:
                x = self.hedge.position()[1]
                y = self.hedge.position()[2]
                z = self.hedge.position()[3]
                elapsed_time = time() - self.start_time

                pos_str = f"{x},{y},{z},{elapsed_time:.2f}"
                self.publisher_.publish(String(data=pos_str))
                self.get_logger().info(f"Published GPS: {pos_str}")
